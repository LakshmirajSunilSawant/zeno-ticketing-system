...
**Live:** `https://<your-service>.onrender.com` · **Interactive docs:** `/docs` · **Health:** `/health`

```bash
curl https://<your-service>.onrender.com/api/v1/events
```
...
Deployed on **Render** from [`render.yaml`](render.yaml) — the database, the service and every
environment variable are declared in code, so the whole stack is reproducible from git.

1. Push to GitHub.
2. Render → **New → Blueprint** → select the repo. It reads `render.yaml` and provisions the web
   service and a Postgres instance together.
3. Set `ADMIN_EMAIL` / `ADMIN_PASSWORD` in the dashboard (they're `sync: false`, so they never
   touch git). `DATABASE_URL` is injected from the database; `JWT_SECRET` is generated by Render.
4. Verify: `/health`, `/docs`, `GET /api/v1/events`.

| Variable | Source | Purpose |
|---|---|---|
| `DATABASE_URL` | injected by Render | Postgres connection (legacy `postgres://` auto-normalised) |
| `JWT_SECRET` | generated by Render | token signing |
| `ENV` | `production` | switches logs to JSON |
| `CORS_ORIGINS` | `*` | callable from anywhere |
| `RATE_LIMIT` | `100/minute` | |
| `ADMIN_EMAIL` / `ADMIN_PASSWORD` | set manually | seeded admin account |

`seed.py` runs on every boot and is idempotent, so the live demo always has an admin and sample
events without a manual step.

> **Free-tier cold start:** the instance sleeps after ~15 minutes idle and takes ~50s to wake.
> Hit `/health` a few minutes before you need it.
